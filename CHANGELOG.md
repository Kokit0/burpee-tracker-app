...
## [2.0.0] - Backend Serverless, Autenticaci�n y Registros Colaborativos - 2026-08-18

### A�adido
- **API REST con Google Apps Script**: Migraci�n total de la base de datos de una lectura est�tica de CSV a una API din�mica capaz de recibir peticiones POST y GET con manejo nativo de CORS.
- **Sistema de Cuentas de Usuario**: Implementaci�n de un modelo de autenticaci�n y auto-registro (LoginModal) para clientes, permitiendo que aporten sus propios n�meros a la meta grupal.
- **Formulario In-App (Aportes)**: Se a�adi� un componente seguro AddModal para el ingreso de registros diarios (clase, fecha, burpees, squats) que actualiza la base de datos en tiempo real.
- **Autor�a de Registros**: Historial individual en la pesta�a *�ltimos Registros* ahora identifica nominalmente a los usuarios aportantes, ocultando inteligentemente los ingresos correspondientes a la cuenta Admin (Ingreso Complemento).
...
- **Paridad de Fechas ISO**: Modernizaci�n de la funci�n parseDate para normalizar cadenas ISO 8601 (T00:00:00.000Z) despachadas por la nueva API.
